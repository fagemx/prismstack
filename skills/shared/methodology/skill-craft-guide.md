...
**品質級別適用性：**
- **Draft（12-17 分）**→ 跳過壓力測試。先搭出來能跑再說。
- **Usable（18-23 分）**→ 做 1 輪 RED-GREEN 就好。不需要 REFACTOR 輪。
- **Production（24-30 分）**→ 完整 3 輪 + Rationalization Prevention 三層。

### 10. 用辯論生成更好的 Skill 內容

Gotchas、scoring formula、role identity 不應該由一個 agent 獨斷。用辯論流程產出更好的版本。

**適用時機：** 生成 skill 的核心內容（gotchas、scoring、forcing questions）時，特別是：
- 用戶沒有提供詳細 spec（需要 AI 自己推導）
- 領域是 AI 不太熟的（容易漏盲點）
- 內容有主觀成分（權重分配、角色定義）

**辯論生成 4 步：**

1. **Author-A** — 第一個 agent 產出初版內容（例：scoring formula 草稿）
2. **Critic** — 強制對抗：找 Author-A 的盲點、漏洞、權重偏差
   - 必須找出至少 2 個具體問題（不能說「整體不錯」）
   - 問：「這個公式漏了什麼維度？」「這個權重為什麼是 30% 不是 20%？」
3. **Author-B** — 看到 A 的版本 + Critic 的攻擊，產出替代版本
   - 不是修補 A，是獨立產出（可能結構完全不同）
4. **Synthesize** — 合成最終版本：取 A 和 B 各自的強項
   - 如果 A 和 B 差異很大 → 標記為 Taste Decision（交給用戶選）
   - 如果 A 和 B 趨同 → 直接合成

**關鍵規則：**
- Author-B 必須看到 Critic 的攻擊（不是盲審，是有意對抗）
- Critic 不能是 Author-A（避免自我辯護）
- 合成時必須說明「從 A 取了什麼、從 B 取了什麼、為什麼」

**不用辯論的情況：**
- 用戶已經提供了完整的 scoring formula → 直接用（P1 保留用戶意圖）
- 機械內容（YAML 格式、file naming）→ 沒有辯論空間
- 只有一個合理答案（例：artifact type 命名）→ 直接做

**範例：生成廣告審查 skill 的 scoring formula**

```
Author-A: 構圖 30% / CTA 25% / 品牌一致 20% / 色彩 15% / 文字 10%
Critic:  「構圖 30% 太高了。手機上用戶先看文字和 CTA，構圖是次要。
          而且缺了『可讀性』維度 — 文字被背景蓋住是常見問題。」
Author-B: CTA 30% / 可讀性 25% / 品牌一致 20% / 構圖 15% / 色彩 10%
Synthesize: CTA 25% / 構圖 20% / 可讀性 20% / 品牌一致 20% / 色彩 15%
  取 A 的構圖（降權但保留）+ 取 B 的可讀性（新增維度）+ 平衡兩者的 CTA 權重
```
